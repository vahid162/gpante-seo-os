...
def _validate_instances(result: ValidationResult, schemas: dict[str, dict[str, Any]], registry: Registry) -> None:
    ...
    common_schema_has_errors = (Path("schemas") / "common.schema.json").as_posix() in schema_error_paths
    for record in result.records:
        ...
        if not schema:
            ...
            continue
        if common_schema_has_errors or (Path("schemas") / record.schema_name).as_posix() in schema_error_paths:
            continue
